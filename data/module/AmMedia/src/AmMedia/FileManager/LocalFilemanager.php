...
class LocalFilemanager extends BaseFilemanager
{
    ...
    public function move()
    {
        ...
        // check if file is writable
        if (!$this->has_system_permission($oldPath, ['w']) || !$this->has_system_permission($newPath, ['w'])) {
            $this->error(sprintf($this->lang('NOT_ALLOWED_SYSTEM')));
        }
    ...
    protected function get_file_info($relative_path, $thumbnail = false)
    {
        ...
        $protected = $this->has_system_permission($current_path, ['w', 'r']) ? 0 : 1;
        ...
        if (is_dir($current_path)) {
            ...
            if ($protected == 1) {
                ...
                } elseif (file_exists($this->fm_path . '/' . $this->config->icons['path'] . strtolower($fileType) . '.png')) {
    ...
    protected function loadLanguageFile()
    {
        ...
        if (file_exists($this->fm_path . '/languages/' . $lang . '.json')) {
            $stream = file_get_contents($this->fm_path . '/languages/' . $lang . '.json');
            $this->language = json_decode($stream, true);
        } else {
            $l = substr($lang, 0, 2); // we try with 2 chars language file
            if (file_exists($this->fm_path . '/languages/' . $l . '.json')) {
                $stream = file_get_contents($this->fm_path . '/languages/' . $l . '.json');
                $this->language = json_decode($stream, true);
            } else {
    ...
    {
        if ($handle = opendir($this->fm_path . '/languages/')) {
            while (false !== ($file = readdir($handle))) {
                if ($file != '.' && $file != '..') {
