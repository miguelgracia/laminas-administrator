...
use AmTool\Diagnostics\Exception\RuntimeException;
use AmTool\Diagnostics\Reporter\BasicConsole;
use AmTool\Diagnostics\Reporter\VerboseConsole;
use AmTool\Diagnostics\Runner;
    ...
    public function runAction()
    {
        ...
        foreach ($config as $checkGroupName => $checks) {
            foreach ($checks as $checkLabel => $check) {
                ...
                // Try to use the AmTool namespace
                } elseif (is_string($testName) && class_exists('AmTool\\Diagnostics\\Check\\' . $testName)) {
                    $class = new \ReflectionClass('AmTool\\Diagnostics\\Check\\' . $testName);
                    $check = $class->newInstanceArgs($params);
